...
int main()
{
    ...
    for (int i = 0; i < grupo1.index; i++)
    {
        ...
        if (persona->imc->peso_clasif == "Bajo peso.")
            conteo[0]++;
        else if (persona->imc->peso_clasif == "Peso normal.")
            conteo[1]++;
        else if (persona->imc->peso_clasif == "Sobrepeso.")
            conteo[2]++;
        else
            conteo[3]++;

        sumaIMC += persona->imc->imc;
        (persona->getSexo() == 1) ? hombres++ : mujeres++;
        if (persona->getEdad() < 18)
            edades[0]++;
        else if (persona->getEdad() <= 35)
            edades[1]++;
        else if (persona->getEdad() <= 50)
            edades[2]++;
        else
            edades[3]++;
    }
